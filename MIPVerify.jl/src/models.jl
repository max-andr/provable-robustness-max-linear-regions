...
@auto_hash_equals struct L2NormBoundedPerturbationFamily <: RestrictedPerturbationFamily
    norm_bound::Real

    function L2NormBoundedPerturbationFamily(norm_bound::Real)
        @assert(
            norm_bound > 0,
            "Norm bound $(norm_bound) should be positive"
        )
...
Base.show(io::IO, pp::L2NormBoundedPerturbationFamily) = print(io, "l2-norm-bounded-$(pp.norm_bound)")
...
function build_reusable_model_uncached(
    nn::NeuralNet,
    input::Array{<:Real},
    pp::L2NormBoundedPerturbationFamily,
    tightening_solver::MathProgBase.SolverInterface.AbstractMathProgSolver,
    tightening_algorithm::TighteningAlgorithm
    )::Dict
...
#     error("stop: $m $tpof")
    ...
    input_range = CartesianRange(size(input))  # for every triple of indices (to iterate over them in map())
    v_e = map(_ -> @variable(m, lowerbound = -pp.norm_bound, upperbound = pp.norm_bound), input_range) # perturbation added
    v_x0 = map(i -> @variable(m, lowerbound = max(0, input[i] - pp.norm_bound), upperbound = min(1, input[i] + pp.norm_bound)), input_range) # perturbation + original image
    @constraint(m, sum(v_e.*v_e) <= pp.norm_bound^2)
    @constraint(m, v_x0 .== input + v_e)
