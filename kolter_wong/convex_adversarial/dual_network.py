...
class DualNetwork(nn.Module):
    def __init__(self, net, X, epsilon, q_norm,
        ...
        # The only modification for L2 goes here
        dual_net = [select_input(X, epsilon, l1_proj, l1_type, bounded_input, q_norm)]
...
def robust_loss_with_point_errors(net, epsilon, X, y,
                size_average=True, device_ids=None, parallel=False, **kwargs):
    if parallel:
        f = nn.DataParallel(RobustBounds(net, epsilon, **kwargs))(X, y)
    else:
        f = RobustBounds(net, epsilon, **kwargs)(X, y)
    is_kw_non_robust = (f.max(1)[1] != y)
    if size_average:
        err = is_kw_non_robust.sum().item() / X.size(0)
    ce_loss = nn.CrossEntropyLoss(reduce=size_average)(f, y)
    return ce_loss, err, ~is_kw_non_robust
