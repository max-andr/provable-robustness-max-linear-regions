...
def train_robust(loader, model, opt, epsilon, epoch, log, verbose,
    ...
    for i, (X, y) in enumerate(loader):
        ...
        if verbose and (i % verbose == 0 or real_time):
            print(epoch, i, robust_ce.detach().item(), robust_err, ce.item(), err.item(), file=log)
        ...
        if DEBUG and i == 10:
            break
    print_msg = '[Train]  Epoch {epoch}, error {error.avg:.3f}, rob_error {rerror.avg:.3f}, loss {loss.avg:.3f}, ' \
                'rob_loss {rloss.avg:.3f}, time {batch_time.sum:.3f}' \
        .format(epoch=epoch, rerror=robust_errors, error=errors, loss=losses, rloss=robust_losses, batch_time=batch_time)
    print(print_msg)
    print(print_msg, file=log)
    torch.cuda.empty_cache()


def evaluate_robust(loader, model, epsilon, epoch, log, verbose,
                    real_time=False, parallel=False, **kwargs):
    n_pts_subset_eval = 1000
    ...
    for i, (X, y) in enumerate(loader):
        ...
        if parallel:
            ...
            robust_ce, robust_err, is_kw_robust_batch = robust_loss_with_point_errors(model, epsilon, X, y, **kwargs)
        ...
        robust_errors.update(robust_err, X.size(0))
        if X.size(0) * (i+1) <= n_pts_subset_eval:
            robust_errors_1k.update(robust_err, X.size(0))
            is_kw_robust_list.append(is_kw_robust_batch)
    ...
    is_kw_robust = torch.cat(is_kw_robust_list).cpu().numpy().astype(bool)
    print_msg = '[Test]  Epoch {epoch}, error {error.avg:.3f}, rob_error {rerror.avg:.3f}, rob_error_1k {rerror_1k.avg:.3f}, loss {loss.avg:.3f}, ' \
                'rob_loss {rloss.avg:.3f}, time {batch_time.sum:.3f}' \
        .format(epoch=epoch, rerror=robust_errors, rerror_1k=robust_errors_1k, error=errors, loss=losses, rloss=robust_losses, batch_time=batch_time)
    print(print_msg)
    print(print_msg, file=log)
    return robust_errors.avg, is_kw_robust
    ...
    for i, (X, y) in enumerate(loader):
        ...
        if verbose and i % verbose == 0:
            ...
            print('Epoch: [{0}][{1}/{2}]\t'
                ...
                data_time=data_time, loss=losses, errors=errors))
        log.flush()
    print_msg = '[Train] Epoch {epoch}, error {error.avg:.3f}, loss {loss.avg:.3f}, time {batch_time.sum:.3f}' \
        .format(epoch=epoch, error=errors, loss=losses, batch_time=batch_time)
    print(print_msg)
    print(print_msg, file=log)
...
def evaluate_baseline(loader, model, epoch, log, verbose):
    ...
    print_msg = '[Test] Epoch {epoch}, error {error.avg:.3f}, loss {loss.avg:.3f}, time {batch_time.sum:.3f}' \
        .format(epoch=epoch, error=errors, loss=losses, batch_time=batch_time)
    print(print_msg)
    print(print_msg, file=log)
    return errors.avg
...
def sampler_robust_cascade(loader, models, epsilon, **kwargs):
    ...
    if len(l) > 0:
        ...
        return torch.utils.data.DataLoader(dataset, batch_size=loader.batch_size, shuffle=False, pin_memory=True,
                                           sampler=sampler)
    else:
        return None
